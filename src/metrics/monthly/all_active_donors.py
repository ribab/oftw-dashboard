#!/usr/bin/env python3
# %%
# Total Number of Active Donors Bar Chart
# This notebook creates a bar chart showing the total number of unique donors with active pledges at the end of each month.
...
def custom_chart(data_frame):
    ...
    # Create list #1: pledge_starts_at and 1 for each unique donor
    created_list = pd.DataFrame({
        'date': data_frame['pledge_starts_at'],
        'donor_id': data_frame['donor_id'],
        'change': 1
    ...
    ended_pledges = data_frame[data_frame['pledge_ended_at'].notna() & (data_frame['pledge_ended_at'] != 'NaT')].copy()
    ...
    # Add a horizontal line at y=1200 (based on requirements)
    fig.add_trace(go.Scatter(
        x=monthly_df['month_label'],
        y=[1200] * len(monthly_df),
        mode='lines',
        name='Goal: 1,200 active donors',
    ...
    fig.update_layout(
        title={
            'text': 'Active Donors at Month End<br><i style="font-size:14px">Includes both one-time donors and active recurring donors</i>',
            'y': 0.95,
            'x': 0.5,
            'xanchor': 'center',
            'yanchor': 'top',
        ...
        xaxis_title='Month',
        yaxis_title='Number of Active Donors',
